...
class FileService:
    ...
    async def save_avatar(self, file: UploadFile, user_id: str) -> str:
        ...
        if not validate_file_extension(file.filename, self.allowed_extensions):
            ...
            allowed_list = ", ".join(
                [ext.strip() for ext in self.allowed_extensions.split(",")])
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail=f"Invalid file type. Allowed formats: {allowed_list}."
            )

        # We will enforce size during streaming copy to avoid unreliable
        # seek/tell on temp files
        ...
        # Return a relative path from the static mount, e.g.,
        # 'avatars/<filename>'
