...
class UserService:
    ...
    def _construct_avatar_url(
        ...
        if user_avatar.startswith(
                "http://") or user_avatar.startswith("https://"):
            return user_avatar

        # If a relative path slipped in, attempt to construct a URL if file
        # exists
    ...
    async def update_me(
        ...
        try:
            ...
            if model_data.get("avatar") == "":
                if user.avatar:
                    # Delete by filename regardless of stored format (URL or
                    # relative)
                    self.files.delete_avatar(user.avatar)
                user.avatar = None

            if avatar:
                # Delete old avatar if exists (works for full URL or relative
                # path)
                if user.avatar:
                    self.files.delete_avatar(user.avatar)

                # Save new avatar (returns relative path like
                # 'avatars/<filename>')
            ...
            # Propagate known client errors (e.g., 400 invalid type, 413 too
            # large)
